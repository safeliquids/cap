 ...
 * When encountering a parse-time error, the parser is able to print a help
 * message before exiting the program. By default, this message is automatically
 * generated on-demand based on configured flags and positionals, but it can be
 * explicitly changed to anything the user wants. That is done using
 * `cap_parser_set_custom_help`. The automatic help looks similar to this.
 * ```
 * usage:
 ...
 * ```
 * If the help message is automatic, it shows the usage string at the top.
 * Similarly to the help text, the usage string is automatically generated by
 * default, but can be manuyally set using `cap_parser_set_custom_usge`.
 * The help and usage texts can be independently enabled or disabled using
 * `cap_parser_enable_help` and `cap_parser_enable_usage`.
 * 
 * ## Parsing
 * 
 * A configured `ArgumentParser` can be used to parse actual command line words.
 * Parsing does not change the parser, and it is safe to parse multiple sets of
 * words with the same parser. Parsing is done using the `cap_parser_parse`
 * function. This function takes command line arguments in the same way as the
 * usual `main` function - a number and an array of strings. The first word (at
 * index 0) should be the name of the program as it was called.
 * 
 * ### Parsing Behaviour
 * There are two modes in which parsing can operate: **normal** and
 * **positional-only**. In **normal** mode, the parser tries to sequentially
 * consume all words given to it, starting with the second word. The normal
 * behaviour is that every word beginning with a flag prefix character is
 * treated as a flag name. The parser then attempts to identify the flag that
 * this name references. If it is found the next step depends on the flag's data
 * type. Presence-type flags do not take any values, so parsign the flag ends
 * there. Else, the next word is parsed according to the flag's type and stored
 * as a value. 
 * 
 * If the parser encounters a word which does not begin with a flag prefix, it
 * is treated as a positional argument. The parsing algorithm remembers how many
 * positionals have been parsed already and which one is next to get a value
 * assigned to it. Do note that, the last positional can be variadic, meaning it
 * can consume up to any number of values.
 * 
 * It is possible to switch from normal to **positoinal-only** mode by using a
 * special flag - the flag separator. That happens when this special token is
 * found in place of a regular flag. In positional-only mode, all words are
 * treated as positionals, even if they begin with a flag prefix characters. For
 * example, this allows for negative numbers to be used as positionals in the
 * default configuration, even though the minus sign '-' is a flag prefix
 ...
 * 2. The flag should take a value, but there are no more words available.
 * 3. There is an available word, but it cannot be parsed as the required type.
 * 4. The flag has already been present the maximum allowed number of times.
 * 
 * When parsing positionals (in either mode), one of these errors can occur:
 * 1. There are too many words that would be treated as positionals, or
 * 2. the current word cannot be parsed as the required type.
 * 
 * Additionally, parsing fails if any flag was not present enough times as per
 * configuration, or some required positonals were not assigned a value.
 * 
 * TODO
 * 
 */
